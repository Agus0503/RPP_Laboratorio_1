 ...
 * @param	cadena espacio de memoria donde se dejara el resultado de la funcion
 * @param	sizeCadena Es la longitud del array resultado
 * @param	mensaje Es el mensaje a ser mostrado
 * @param	mensajeError Es el mensaje de Error a ser mostrado
 * @return	Retorna 1 si se obtuvo el array y -1 si no
 *
...
int obtenerCadenaAlfanumerica(char cadena[], int sizeCadena, char* mensaje, char* mensajeError)
{
	...
	if(cadena != NULL && mensaje != NULL && mensaje != NULL && sizeCadena > 0)
	{
		do{
			printf("%s", mensaje);
			fflush(stdin);
